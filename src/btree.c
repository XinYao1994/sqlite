...
static int querySharedCacheTableLock(Btree *p, Pgno iTab, u8 eLock) {
	...
	assert(p->db != 0);
	assert(
			!(p->db->flags & SQLITE_ReadUncommit) || eLock == WRITE_LOCK
					|| iTab == 1);
	...
	for (pIter = pBt->pLock; pIter; pIter = pIter->pNext) {
		...
		assert(
				eLock == READ_LOCK
						|| pIter->pBtree == p|| pIter->eLock==READ_LOCK);
...
static int btreeGetHasContent(BtShared *pBt, Pgno pgno) {
	...
	return p
			&& (pgno > sqlite3BitvecSize(p) || sqlite3BitvecTestNotNull(p, pgno));
}
 ...
 */
static int SQLITE_NOINLINE saveCursorsOnList(BtCursor *p, /* The first cursor that needs saving */
...
static void ptrmapPut(BtShared *pBt, Pgno key, u8 eType, Pgno parent, int *pRC) {
	...
	ptrmap_exit: sqlite3PagerUnref(pDbPage);
}
 ...
 ** structure.
 */
static SQLITE_NOINLINE void btreeParseCellAdjustSizeForOverflow(MemPage *pPage, /* Page containing the cell */
...
CellInfo *pInfo /* Fill in this structure */
	...
	surplus = minLocal
			+ (pInfo->nPayload - minLocal) % (pPage->pBt->usableSize - 4);
	testcase( surplus==maxLocal ); testcase( surplus==maxLocal+1 );
	if (surplus <= maxLocal) {
 ...
 ** by pointer.
 */
static void btreeParseCellPtrNoPayload(MemPage *pPage, /* Page containing the cell */
u8 *pCell, /* Pointer to the cell text. */
CellInfo *pInfo /* Fill in this structure */
) {
...
CellInfo *pInfo /* Fill in this structure */
	...
	testcase( nPayload==pPage->maxLocal ); testcase( nPayload==(u32)pPage->maxLocal+1 );
...
static void btreeParseCellPtrIndex(MemPage *pPage, /* Page containing the cell */
u8 *pCell, /* Pointer to the cell text. */
CellInfo *pInfo /* Fill in this structure */
) {
	u8 *pIter; /* For scanning through pCell */
...
static u16 cellSizePtr(MemPage *pPage, u8 *pCell) {
  ...
  ** this function verifies that this invariant is not violated. */
  ...
  pPage->xParseCell(pPage, pCell, &debuginfo);
	...
	pIter++;
	testcase( nSize==pPage->maxLocal ); testcase( nSize==(u32)pPage->maxLocal+1 );
...
static u16 cellSizePtrTableLeaf(MemPage *pPage, u8 *pCell) {
  ...
  ** this function verifies that this invariant is not violated. */
  ...
  pPage->xParseCell(pPage, pCell, &debuginfo);
	...
	if ((*pIter++) & 0x80 && (*pIter++) & 0x80 && (*pIter++) & 0x80
			&& (*pIter++) & 0x80 && (*pIter++) & 0x80 && (*pIter++) & 0x80
			&& (*pIter++) & 0x80 && (*pIter++) & 0x80) {
		pIter++;
	} testcase( nSize==pPage->maxLocal ); testcase( nSize==(u32)pPage->maxLocal+1 );
	if (nSize <= pPage->maxLocal) {
		nSize += (u32) (pIter - pCell);
...
static int defragmentPage(MemPage *pPage, int nMaxFrag) {
			...
			if (0 == iFree2 || (data[iFree2] == 0 && data[iFree2 + 1] == 0)) {
				...
				if (iFree2) {
					...
					if (iFree2 + sz2 > usableSize)
						return SQLITE_CORRUPT_PAGE(pPage);
					memmove(&data[iFree + sz + sz2], &data[iFree + sz],
							iFree2 - (iFree + sz));
					sz += sz2;
	...
	for (i = 0; i < nCell; i++) {
		...
		pc = get2byte(pAddr);
		testcase( pc==iCellFirst ); testcase( pc==iCellLast );
		...
		testcase( cbrk+size==usableSize ); testcase( pc+size==usableSize );
		put2byte(pAddr, cbrk);
		if (temp == 0) {
			if (cbrk == pc)
				continue;
			temp = sqlite3PagerTempSpace(pPage->pBt->pPager);
			memcpy(&temp[iCellStart], &data[iCellStart],
					usableSize - iCellStart);
			src = temp;
		}
	...
	defragment_out: assert(pPage->nFree >= 0);
...
static int allocateSpace(MemPage *pPage, int nByte, int *pIdx) {
	 ...
	 */
	testcase( gap+2==top ); testcase( gap+1==top ); testcase( gap==top );
...
static int btreeInitPage(MemPage *pPage) {
	...
	assert(
			pPage->nCell > 0 || get2byteNotZero(&data[5])==(int)pBt->usableSize
			|| CORRUPT_DB);
...
int flags /* PAGER_GET_NOCONTENT or PAGER_GET_READONLY */
	...
	assert(
			flags
					== 0|| flags==PAGER_GET_NOCONTENT || flags==PAGER_GET_READONLY);
	assert(sqlite3_mutex_held(pBt->mutex));
...
int bReadOnly /* True for a read-only page */
	...
	*ppPage = (MemPage*) sqlite3PagerGetExtra(pDbPage);
	...
	getAndInitPage_error2: releasePage(*ppPage);
	getAndInitPage_error1: if (pCur) {
		pCur->iPage--;
		pCur->pPage = pCur->apPage[pCur->iPage];
	} testcase( pgno==0 );
	assert(
			pgno != 0|| rc==SQLITE_CORRUPT
 ...
 */
static int btreeGetUnusedPage(BtShared *pBt, /* The btree */
Pgno pgno, /* Number of the page to fetch */
MemPage **ppPage, /* Return the page in this parameter */
int flags /* PAGER_GET_NOCONTENT or PAGER_GET_READONLY */
) {
 ...
 */
int sqlite3BtreeOpen(sqlite3_vfs *pVfs, /* VFS to use for this b-tree */
  ...
  const int isMemdb = 0;
	...
	if (isTempDb == 0 && (isMemdb == 0 || (vfsFlags & SQLITE_OPEN_URI) != 0)) {
		if (vfsFlags & SQLITE_OPEN_SHAREDCACHE) {
			...
			if (isMemdb) {
				...
				rc = sqlite3OsFullPathname(pVfs, zFilename, nFullPathname,
						zFullPathname);
				if (rc) {
...
#endif
			for (pBt = GLOBAL(BtShared*, sqlite3SharedCacheList); pBt;
					pBt = pBt->pNext) {
				assert(pBt->nRef > 0);
				if (0
						== strcmp(zFullPathname,
								sqlite3PagerFilename(pBt->pPager, 0))
	...
	if (pBt == 0) {
		...
		if (rc == SQLITE_OK) {
			...
			rc = sqlite3PagerReadFileheader(pBt->pPager, sizeof(zDbHeader),
					zDbHeader);
		}
		if (rc != SQLITE_OK) {
			goto btree_open_out;
	...
	*ppBtree = p;
	...
	btree_open_out: if (rc != SQLITE_OK) {
		if (pBt && pBt->pPager) {
...
int sqlite3BtreeSetPagerFlags(Btree *p, /* The btree to set the safety level on */
...
static int lockBtree(BtShared *pBt) {
	...
	if (nPage == 0
			|| memcmp(24 + (u8*) pPage1->aData, 92 + (u8*) pPage1->aData, 4)
					!= 0) {
		nPage = nPageFile;
	}
    ...
    if( page1[19]>1 ){
      goto page1_init_failed;
		...
		if (((pageSize - 1) & pageSize) != 0 || pageSize > SQLITE_MAX_PAGE_SIZE
				|| pageSize <= 256) {
			goto page1_init_failed;
		}
		pBt->btsFlags |= BTS_PAGESIZE_FIXED;
	...
	page1_init_failed: releasePageOne(pPage1);
	pBt->pPage1 = 0;
	return rc;
}
      ...
      if( rc!=SQLITE_BUSY && rc!=SQLITE_OK ) break;
		...
		if (rc == SQLITE_OK && wrflag) {
			if ((pBt->btsFlags & BTS_READ_ONLY) != 0) {
				...
				rc = sqlite3PagerBegin(pPager, wrflag > 1,
		...
		if (rc != SQLITE_OK) {
			...
			&& btreeInvokeBusyHandler(pBt)); sqlite3PagerWalDb(pPager, 0);
#ifdef SQLITE_ENABLE_SETLK_TIMEOUT
	...
	if (eType == PTRMAP_OVERFLOW2) {
		...
		for (i = 0; i < nCell; i++) {
			...
			if (eType == PTRMAP_OVERFLOW1) {
				...
				if (info.nLocal < info.nPayload) {
					if (pCell + info.nSize
							> pPage->aData + pPage->pBt->usableSize) {
						return SQLITE_CORRUPT_PAGE(pPage);
					}
...
int isCommit /* isCommit flag passed to sqlite3PagerMovepage */
	...
	assert(
			eType == PTRMAP_OVERFLOW2 || eType == PTRMAP_OVERFLOW1
...
static int autoVacuumCommit(Btree *p) {
	...
	if (!pBt->incrVacuum) {
		...
		if (db->xAutovacPages) {
			...
			nVac = db->xAutovacPages(db->pAutovacPagesArg,
					db->aDb[iDb].zDbSName, nOrig, nFree, pBt->pageSize);
			if (nVac > nFree) {
				nVac = nFree;
			}
			if (nVac == 0) {
...
BtCursor *pCur /* Space for new cursor */
	...
	assert(
			hasSharedCacheTableLock(p, iTable, pKeyInfo != 0, (wrFlag ? 2 : 1))
					|| iTable < 1);
...
static int btreeCursorWithLock(Btree *p, /* The btree */
Pgno iTable, /* Root page of table to open */
int wrFlag, /* 1 to write. 0 read-only */
struct KeyInfo *pKeyInfo, /* First arg to comparison function */
BtCursor *pCur /* Space for new cursor */
...
int sqlite3BtreeCursor(Btree *p, /* The btree */
Pgno iTable, /* Root page of table to open */
int wrFlag, /* 1 to write. 0 read-only */
  ...
  unsigned char * const pBufStart = pBuf;     /* Start of original out buffer */
	...
	if (rc == SQLITE_OK && amt > 0) {
		...
		if ((pCur->curFlags & BTCF_ValidOvfl) == 0) {
			int nOvfl = (pCur->info.nPayload - pCur->info.nLocal + ovflSize - 1)
					/ ovflSize;
			if (pCur->aOverflow == 0
					|| nOvfl * (int) sizeof(Pgno)
							> sqlite3MallocSize(pCur->aOverflow)) {
				Pgno *aNew = (Pgno*) sqlite3Realloc(pCur->aOverflow,
						nOvfl * 2 * sizeof(Pgno));
				if (aNew == 0) {
					return SQLITE_NOMEM_BKPT;
				} else {
		...
		while (nextPage) {
			...
			assert(
					pCur->aOverflow[iIdx]
							== 0 || pCur->aOverflow[iIdx]==nextPage
							|| CORRUPT_DB);
 ...
 */
#ifndef SQLITE_OMIT_INCRBLOB
static SQLITE_NOINLINE int accessPayloadChecked(BtCursor *pCur, u32 offset,
		u32 amt, void *pBuf) {
	int rc;
 ...
 */
static const void* fetchPayload(BtCursor *pCur, /* Cursor pointing to entry to read from */
...
static int moveToRoot(BtCursor *pCur) {
	...
	if (pCur->iPage >= 0) {
		...
		rc = getAndInitPage(pCur->pBtree->pBt, pCur->pgnoRoot, &pCur->pPage, 0,
				pCur->curPagerFlags);
		if (rc != SQLITE_OK) {
			pCur->eState = CURSOR_INVALID;
			return rc;
...
int *pRes /* Write search results here */
  ...
  pCur->pBtree->nSeek++;   /* Performance measurement during testing */
	...
	for (;;) {
		...
		// xyao
		btree_txn_log_range(pCur, pPage);
		//
		...
		moveto_table_next_layer: if (lwr >= pPage->nCell) {
	...
	moveto_table_finish: pCur->info.nSize = 0;
...
int *pRes /* Write search results here */
  ...
  pCur->pBtree->nSeek++;   /* Performance measurement during testing */
	...
	pIdxKey->errCode = 0;
	assert(
			pIdxKey->default_rc == 1 || pIdxKey->default_rc == 0
	...
	for (;;) {
		...
		for (;;) {
			...
			if (nCell <= pPage->max1bytePayload) {
				...
				pCur->ix = (u16) idx;
				rc = accessPayload(pCur, 0, nCell, (unsigned char*) pCellKey,
						0);
	...
	moveto_index_finish: pCur->info.nSize = 0;
	assert((pCur->curFlags & BTCF_ValidOvfl) == 0);
	return rc;
}
...
int sqlite3BtreePrevious(BtCursor *pCur, int flags) {
	...
	pCur->curFlags &= ~(BTCF_AtLast | BTCF_ValidOvfl | BTCF_ValidNKey);
	pCur->info.nSize = 0;
	if (pCur->eState != CURSOR_VALID || pCur->ix == 0
			|| pCur->pPage->leaf == 0) {
		// xyao
 ...
 */
static int allocateBtreePage(BtShared *pBt, /* The btree */
...
u8 eMode /* BTALLOC_EXACT, BTALLOC_LT, or BTALLOC_ANY */
	...
	assert(
			eMode == BTALLOC_ANY
					|| (nearby > 0 && IfNotOmitAV(pBt->autoVacuum)));
	pPage1 = pBt->pPage1;
	...
	if (n > 0) {
			...
			if (k == 0 && !searchList) {
				...
				*ppPage = pTrunk;
				...
				goto end_allocate_page;
					...
					&& (nearby == iTrunk
							|| (iTrunk < nearby && eMode == BTALLOC_LE))) {
				...
				*ppPage = pTrunk;
				...
				if (nearby > 0) {
					...
					if (eMode == BTALLOC_LE) {
						...
						for (i = 1; i < k; i++) {
							int d2 = sqlite3AbsInt32(
									get4byte(&aData[8 + i * 4]) - nearby);
							if (d2 < dist) {
								closest = i;
								dist = d2;
							}
		 ...
		 */
		int bNoContent =
				(0 == IfNotOmitAV(pBt->bDoTruncate)) ? PAGER_GET_NOCONTENT : 0;
	...
	end_allocate_page: releasePage(pTrunk);
	releasePage(pPrevTrunk);
	assert(
			rc != SQLITE_OK
 ...
 */
static SQLITE_NOINLINE int clearCellOverflow(MemPage *pPage, /* The page that contains the Cell */
...
CellInfo *pInfo /* Size information about the cell */
	...
	testcase( pCell + pInfo->nSize == pPage->aDataEnd ); testcase( pCell + (pInfo->nSize-1) == pPage->aDataEnd );
...
int *pnSize /* Write cell size here */
	...
	assert(
			pCell < pPage->aData || pCell >= &pPage->aData[pPage->pBt->pageSize]
	...
	testcase( n==pPage->maxLocal ); testcase( n==pPage->maxLocal+1 );
	if (n > pPage->maxLocal)
		n = mn;
	spaceLeft = n;
	*pnSize = n + nHeader + 4;
...
#endif
	testcase( pc==(u32)get2byte(&data[hdr+5]) ); testcase( pc+sz==pPage->pBt->usableSize );
...
int *pRC /* Read and write return code from here */
	...
	if (pPage->nOverflow || sz + 2 > pPage->nFree) {
		...
		assert(
				get2byte(&data[pPage->hdrOffset + 3])
						== pPage->nCell|| CORRUPT_DB);
#ifndef SQLITE_OMIT_AUTOVACUUM
 ...
 */
static int rebuildPage(CellArray *pCArray, /* Content to be added to page pPg */
 ...
 */
static int pageInsertArray(MemPage *pPg, /* Page to add cells to */
...
CellArray *pCArray /* Array of cells */
	...
	while (1 /*Exit by break*/) {
		...
		assert(
				(pSlot + sz)
						<= pCArray->apCell[i] || pSlot>=(pCArray->apCell[i]+sz)
						|| CORRUPT_DB);
 ...
 */
static int pageFreeArray(MemPage *pPg, /* Page to edit */
 ...
 ** responsibility of the caller to set it correctly.
 */
static int editPage(MemPage *pPg, /* Edit this page */
  ...
  memcpy(pTmp, aData, pPg->pBt->usableSize);
	...
	for (i = 0; i < pPg->nOverflow; i++) {
		...
		if (iCell >= 0 && iCell < nNew) {
			...
			if (pageInsertArray(pPg, pBegin, &pData, pCellptr, iCell + iNew, 1,
					pCArray))
				goto editpage_fail;
		}
	}
	...
	if (pageInsertArray(pPg, pBegin, &pData, pCellptr, iNew + nCell,
			nNew - nCell, pCArray))
		goto editpage_fail;
...
int bBulk /* True if this call is part of a bulk load */
	...
	while (1) {
		...
		if (pParent->nOverflow && i + nxDiv == pParent->aiOvfl[0]) {
			...
			if (pBt->btsFlags & BTS_FAST_SECURE) {
				...
				iOff = SQLITE_PTR_TO_INT(
						apDiv[i]) - SQLITE_PTR_TO_INT(pParent->aData);
	...
	assert(
			cntNew[0]
					> 0|| (pParent->pgno==1 && pParent->nCell==0) || CORRUPT_DB);
	...
	for (i = 0; i < k; i++) {
		...
		if (i < nOld) {
			...
			if (sqlite3PagerPageRefcount(pNew->pDbPage)
					!= 1 + (i == (iParentIdx - nxDiv)) && rc == SQLITE_OK) {
				rc = SQLITE_CORRUPT_BKPT;
			}
			if (rc)
				goto balance_cleanup;
	...
	if ( ISAUTOVACUUM) {
		...
		for (i = 0; i < b.nCell; i++) {
			...
			|| !SQLITE_WITHIN(pCell,pOld->aData,pOld->aDataEnd)
				...
				if (!leafCorrection) {
					ptrmapPut(pBt, get4byte(pCell), PTRMAP_BTREE, pNew->pgno,
							&rc);
				}
	...
	for (i = 1 - nNew; i < nNew; i++) {
		...
		|| cntOld[iPg - 1] >= cntNew[iPg - 1] /* Condition (1) is true */
			...
			assert(
					iPg == 0 || cntOld[iPg - 1] >= cntNew[iPg - 1]
							|| abDone[iPg - 1]);
	...
	if (isRoot && pParent->nCell == 0
		...
		assert(
				apNew[0]->nFree
						== (get2byteNotZero(&apNew[0]->aData[5])
								- apNew[0]->cellOffset - apNew[0]->nCell * 2) || rc!=SQLITE_OK
						);
...
static int anotherValidCursor(BtCursor *pCur) {
	...
	for (pOther = pCur->pBt->pCursor; pOther; pOther = pOther->pNext) {
		if (pOther != pCur && pOther->eState == CURSOR_VALID
...
static int balance(BtCursor *pCur) {
	...
	VVA_ONLY( int balance_quick_called = 0 ); VVA_ONLY( int balance_deeper_called = 0 );

	do {
		int iPage;
		MemPage *pPage = pCur->pPage;

		if ( NEVER(pPage->nFree<0) && btreeComputeFreeSpace(pPage))
			break;
		if (pPage->nOverflow == 0 && pPage->nFree <= nMin) {
			...
			if (rc == SQLITE_OK) {
#ifndef SQLITE_OMIT_QUICKBALANCE
				if (pPage->intKeyLeaf && pPage->nOverflow == 1
 ...
 */
static int btreeOverwriteContent(MemPage *pPage, /* MemPage on which writing will occur */
...
int iAmt /* Number of bytes to be written */
	...
	if (nData <= 0) {
		...
		if (nData < iAmt) {
			...
			int rc = btreeOverwriteContent(pPage, pDest + nData, pX,
...
static int btreeOverwriteCell(BtCursor *pCur, const BtreePayload *pX) {
	...
	rc = btreeOverwriteContent(pPage, pCur->info.pPayload, pX, 0,
			pCur->info.nLocal);
	if (rc)
		return rc;
	...
	do {
		...
		if (sqlite3PagerPageRefcount(pPage->pDbPage) != 1 || pPage->isInit) {
			...
			rc = btreeOverwriteContent(pPage, pPage->aData + 4, pX, iOffset,
					ovflPageSize);
		}
 ...
 ** to decode the key.
 */
int sqlite3BtreeInsert(BtCursor *pCur, /* Insert data into the table of this cursor */
...
int seekResult /* Result of prior MovetoUnpacked() call */
	...
	assert(
			(flags & (BTREE_SAVEPOSITION | BTREE_APPEND | BTREE_PREFORMAT))
					== flags);
	...
	assert(cursorOwnsBtShared(pCur));
	assert(
			(pCur->curFlags & BTCF_WriteFlag) != 0
					&& pBt->inTransaction == TRANS_WRITE
	...
	assert(
			(flags & BTREE_PREFORMAT)
					|| (pX->pKey == 0) == (pCur->pKeyInfo == 0));
	...
	if (loc == 0) {
		...
		BTREE_CLEAR_CELL(rc, pPage, oldCell, info); testcase( pCur->curFlags & BTCF_ValidOvfl );
		...
		do {
			...
			do {
				...
				if (nOut > 0) {
					...
					pPageIn = 0;
					rc = sqlite3PagerGet(pSrcPager, ovflIn, &pPageIn,
							PAGER_GET_READONLY);
					if (rc == SQLITE_OK) {
			...
			if (rc == SQLITE_OK && nRem > 0 && ALWAYS(pPgnoOut)) {
				...
				if ( ISAUTOVACUUM && pPageOut) {
					ptrmapPut(pBt, pgnoNew, PTRMAP_OVERFLOW2, pPageOut->pgno,
							&rc);
				}
...
int sqlite3BtreeDelete(BtCursor *pCur, u8 flags) {
	...
	if (pCur->eState == CURSOR_REQUIRESEEK) {
		...
		if (rc || pCur->eState != CURSOR_VALID)
			return rc;
	}
	...
	if (bPreserve) {
		if (!pPage->leaf
				|| (pPage->nFree + pPage->xCellSize(pPage, pCell) + 2)
						> (int) (pBt->usableSize * 2 / 3) || pPage->nCell == 1 /* See dbfuzz001.test for a test case */
				) {
...
static int btreeCreateTable(Btree *p, Pgno *piTable, int createTabFlags) {
  ...
  if( rc ){
	...
	if (pBt->autoVacuum) {
		 ...
		 */
		rc = allocateBtreePage(pBt, &pPageMove, &pgnoMove, pgnoRoot,
				BTALLOC_EXACT);
		if (rc != SQLITE_OK) {
			return rc;
		}
...
i64 *pnChange /* Add number of Cells freed to this counter */
	...
	cleardatabasepage_out: releasePage(pPage);
	return rc;
}
...
static void checkAppendMsg(IntegrityCk *pCheck, const char *zFormat, ...) {
	...
	if (pCheck->zPfx) {
		sqlite3_str_appendf(&pCheck->errMsg, pCheck->zPfx, pCheck->v1,
				pCheck->v2);
	}
...
static int checkRef(IntegrityCk *pCheck, Pgno iPage) {
	...
	if (AtomicLoad(&pCheck->db->u1.isInterrupted))
		return 1;
...
Pgno iParent /* Expected pointer map parent page number */
	...
	if (ePtrmapType != eType || iPtrmapParent != iParent) {
		checkAppendMsg(pCheck,
				"Bad ptr map entry key=%d expected=(%d,%d) got=(%d,%d)", iChild,
				eType, iParent, ePtrmapType, iPtrmapParent);
	}
}
#endif
...
u32 N /* Expected number of pages in the list */
	...
	while (iPage != 0 && pCheck->mxErr) {
		...
		if (isFreeList) {
			...
			if (n > pCheck->pBt->usableSize / 4 - 2) {
				checkAppendMsg(pCheck, "freelist leaf count too big on page %d",
						iPage);
				N--;
			} else {
				for (i = 0; i < (int) n; i++) {
	...
	if (N && nErrAtStart == pCheck->nErr) {
		checkAppendMsg(pCheck, "%s is %d but should be %d",
				isFreeList ? "size" : "overflow list length", expected - N,
				expected);
	}
}
#endif /* SQLITE_OMIT_INTEGRITY_CHECK */
...
i64 maxKey /* Error if integer primary key greater than this */
	...
	for (i = nCell - 1; i >= 0 && pCheck->mxErr; i--) {
		...
		if (pc < contentOffset || pc > usableSize - 4) {
			checkAppendMsg(pCheck, "Offset %d out of range %d..%d", pc,
					contentOffset, usableSize - 4);
			doCoverageCheck = 0;
			continue;
		}
		pCell = &data[pc];
		...
		if (info.nPayload > info.nLocal) {
			...
			nPage = (info.nPayload - info.nLocal + usableSize - 5)
					/ (usableSize - 4);
	...
	*piMinKey = maxKey;
	...
	end_of_check: if (!doCoverageCheck)
 ...
 */
char* sqlite3BtreeIntegrityCheck(sqlite3 *db, /* Database connection that is running the check */
...
int *pnErr /* Write number of errors seen to this variable */
		...
		if (pBt->autoVacuum) {
			...
			if (mx != mxInHdr) {
				checkAppendMsg(&sCheck,
						"max rootpage (%d) disagrees with header (%d)", mx,
						mxInHdr);
			}
	 ...
	 */
	integrity_ck_cleanup: sqlite3PageFree(sCheck.heap);
...
int sqlite3BtreeCheckpoint(Btree *p, int eMode, int *pnLog, int *pnCkpt) {
	...
	if (p) {
		...
		if (pBt->inTransaction != TRANS_NONE) {
			...
			rc = sqlite3PagerCheckpoint(pBt->pPager, p->db, eMode, pnLog,
					pnCkpt);
		}
		sqlite3BtreeLeave(p);
	}
...
int sqlite3BtreePutData(BtCursor *pCsr, u32 offset, u32 amt, void *z) {
	...
	assert(
			(pCsr->pBt->btsFlags & BTS_READ_ONLY)
					== 0 && pCsr->pBt->inTransaction==TRANS_WRITE);
